...
const ExamManager: React.FC<{ courses: Course[], exams: Exam[], addExam: (exam: Omit<Exam, 'id'>) => void, deleteExam: (id: string) => void }> = ({ courses, exams, addExam, deleteExam }) => {
    const [showModal, setShowModal] = useState(false);
    const [title, setTitle] = useState('');
    ...
    useEffect(() => {
        if (courses.length > 0 && results.length === 0) {
            setResults(courses.map(c => ({ courseId: c.id, correct: 0, incorrect: 0, net: 0 })));
        }
    }, [courses]);

    const handleResultChange = (index: number, field: keyof ExamResult, value: number) => {
        const newResults = [...results];
        newResults[index] = { ...newResults[index], [field]: value };
        // Recalculate net: Correct - (Incorrect / 3) or just Correct - Incorrect depending on system. Let's assume standard 3 wrong 1 right.
        if (field === 'correct' || field === 'incorrect') {
            const correct = field === 'correct' ? value : newResults[index].correct;
            const incorrect = field === 'incorrect' ? value : newResults[index].incorrect;
            newResults[index].net = Number((correct - (incorrect / 3)).toFixed(2));
        }
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        const totalNet = results.reduce((acc, curr) => acc + curr.net, 0);
        addExam({
            title,
            date,
            results,
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-md mb-6">
            <div className="flex justify-between items-center mb-4">
                ...
                >
                    <PlusCircle className="w-5 h-5 mr-2" /> Sınav Ekle
                </button>
            </div>

            <Modal show={showModal} onClose={() => setShowModal(false)} title="Yeni Sınav Sonucu Ekle">
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-slate-700">Sınav Adı</label>
                        <input type="text" value={title} onChange={e => setTitle(e.target.value)} required className="w-full border rounded-lg p-2" placeholder="Örn: Okul Denemesi 1" />
                    </div>
                    <div>
                    ...
                    <div className="max-h-60 overflow-y-auto space-y-3">
                        {results.map((res, idx) => (
                            <div key={res.courseId} className="bg-slate-50 p-3 rounded-lg">
                                <p className="font-bold text-sm mb-2">{courses.find(c => c.id === res.courseId)?.name}</p>
                                <div className="flex space-x-2">
                                    <input type="number" placeholder="D" value={res.correct} onChange={e => handleResultChange(idx, 'correct', Number(e.target.value))} className="w-1/3 border rounded p-1 text-center" />
                                    <input type="number" placeholder="Y" value={res.incorrect} onChange={e => handleResultChange(idx, 'incorrect', Number(e.target.value))} className="w-1/3 border rounded p-1 text-center" />
                                    <div className="w-1/3 bg-white border rounded p-1 text-center font-bold text-slate-600">{res.net} Net</div>
                                </div>
                            </div>
                        ))}
                    </div>
                    <button type="submit" className="w-full bg-purple-600 text-white py-2 rounded-lg font-bold">Kaydet</button>
                </form>
            </Modal>
            ...
            {exams.length > 0 ? (
                <div className="space-y-2 max-h-60 overflow-y-auto">
                    {exams.map(exam => (
                        <div key={exam.id} className="flex justify-between items-center p-3 bg-slate-50 rounded-lg border border-slate-100">
                            <div>
                ...
                <p className="text-slate-500 text-center py-4">Henüz sınav sonucu eklenmemiş.</p>
            )}
        </div>
    );
};
...
const PerformanceAnalytics: React.FC<{
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-md">
                ...
                <ResponsiveContainer width="100%" height={300}>
                    <AreaChart data={activityData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                        ...
                        <Tooltip />
                        <Area type="monotone" dataKey="Çalışma Süresi (dk)" stroke="#8884d8" fillOpacity={1} fill="url(#colorDuration)" />
                        <Area type="monotone" dataKey="Soru Sayısı" stroke="#82ca9d" fillOpacity={1} fill="url(#colorQuestions)" />
                    </AreaChart>
                </ResponsiveContainer>
            </div>
            ...
            {/* Weekly Distribution */}
            <div className="bg-white p-6 rounded-xl shadow-md">
                <h3 className="text-xl font-bold mb-4">Ders Bazlı Çalışma Dağılımı (Haftalık)</h3>
                <ResponsiveContainer width="100%" height={300}>
...
const ParentDashboard: React.FC<ParentDashboardProps> = (props) => {
    ...
    const NavLink: React.FC<{ view: ParentView; icon: React.ReactNode; label: string; onClick?: () => void }> = ({ view, icon, label, onClick }) => (
        <button
            ...
            className={`flex items-center w-full text-left px-4 py-3 rounded-lg transition-colors ${activeView === view ? 'bg-primary-600 text-white font-semibold shadow-lg' : 'text-slate-600 hover:bg-primary-100 hover:text-primary-700'
                }`}
            title={label}
            aria-label={label}
        >
