...
import { timerService, TimerState as ServiceTimerState } from '../../services/TimerService';
...
// Local TimerState type (will use ServiceTimerState instead)

interface ActiveTaskTimerProps {
    task: Task;
    tasks: Task[]; // Full task list to check for deletions
    onComplete: (taskId: string, data: TaskCompletionData) => void | (() => void);
    onFinishSession: () => void;
    onPauseForLater: (taskId: string, timerState: ServiceTimerState) => void;
    initialTimerState?: ServiceTimerState;
}
...
const ActiveTaskTimer: React.FC<ActiveTaskTimerProps> = React.memo(({ task, tasks, onComplete, onFinishSession, onPauseForLater, initialTimerState }) => {
    ...
    useEffect(() => {
        ...
        if (!taskStillExists) {
            ...
            timerService.clearTimerState(task.id);
            setWasTaskDeleted(true);
        }
    }, [tasks, task.id]);

    // Timer state initialization from TimerService
    useEffect(() => {
        const savedState = timerService.loadTimerState(task.id);
        if (savedState) {
            setMainTime(savedState.mainTime);
            setBreakTime(savedState.breakTime);
            setPauseTime(savedState.pauseTime);
            setStatus(savedState.status);
            if (savedState.note) {
                setTaskNote(savedState.note);
            }
        }
    }, [task.id]);

    // Timer management with TimerService
    useEffect(() => {
        ...
        // Create state snapshot without triggering new updates
        const currentStateSnapshot: ServiceTimerState = {
            mainTime,
            breakTime,
            pauseTime,
            status,
        ...
        const callbacks = {
            onMainTimeUpdate: (time: number) => setMainTime(time),
            onBreakTimeUpdate: (time: number) => setBreakTime(time),
            onPauseTimeUpdate: (time: number) => setPauseTime(time)
        };

        timerService.startTimer(task.id, currentStateSnapshot, callbacks);

        return () => {
            timerService.stopTimer(task.id);
        };
    }, [status, showCountdown, isCompleted, task.id]); // Only essential dependencies
    ...
    // Safe timer cleanup utility function
    const safeCleanupTimer = () => {
        timerService.stopTimer(task.id);
    };

    // Complete timer reset for task completion
    const resetTimerCompletely = useCallback(() => {
        // Mark timer as permanently destroyed
        isTimerDestroyed.current = true;
        
        // Immediately stop and clear timer service
        timerService.stopTimer(task.id);
        timerService.clearTimerState(task.id);
        ...
        // Don't reset timer values - we need them for completion data!
    }, [task.id]);
    ...
    const handleFinishRequest = () => {
        ...
        } else if (task.courseId === 'deneme-sinavi') {
            // Bu bir sınav, onComplete'i argümansız çağır
            if (typeof onComplete === 'function' && onComplete.length === 0) {
                (onComplete as () => void)();
            }
        } else {
    ...
    const handleSaveNote = (note: string) => {
        // Clean up timer before saving state
        safeCleanupTimer();
        
        const currentTimerState: ServiceTimerState = {
            mainTime,
            breakTime,
            pauseTime,
            status: 'paused', // Ensure status is paused when saving for later
            isPaused: true,
            pausedAt: Date.now(),
            note
        };
        
        // Update timer service state (handles localStorage automatically)
        timerService.updateState(task.id, currentTimerState);
    ...
    const handleConfirmCompletion = () => {
        // Complete timer reset - stops timer, clears state, resets UI
        resetTimerCompletely();
        
        // Firebase için veri hazırlığı - undefined değerleri eklemiyoruz
        ...
        // Sadece soru çözme görevleri için sayı alanlarını ekle
        ...
        if (typeof onComplete === 'function' && onComplete.length > 0) {
            (onComplete as (taskId: string, data: TaskCompletionData) => void)(task.id, completionData);
        }
        onFinishSession();
    };
                 ...
                 <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
                    <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm text-center">
                        ...
                        <div className="mt-6 flex space-x-2">
                             <button onClick={() => {setShowCompleteModal(false); setStatus('running'); timerService.updateStatus(task.id, 'running');}} className="w-full px-4 py-2 text-sm font-semibold text-slate-700 bg-slate-100 rounded-lg hover:bg-slate-200">
                                Geri Dön
                            </button>
            ...
            <div className="fixed inset-0 bg-slate-100 z-40 flex flex-col justify-center items-center p-8">
                <div className="w-full max-w-2xl text-center">
                    ...
                    <div className="flex justify-center items-center space-x-4">
                        {status === 'running' ? (
                            <button onClick={() => {setStatus('paused'); timerService.updateStatus(task.id, 'paused');}} className="flex items-center justify-center w-32 h-16 bg-white border-2 border-gray-300 text-gray-700 font-semibold rounded-lg hover:bg-gray-100"><Pause className="w-6 h-6 mr-2" /> Durdur</button>
                        ) : (
                            <button onClick={() => {setStatus('running'); timerService.updateStatus(task.id, 'running');}} className="flex items-center justify-center w-32 h-16 bg-white border-2 border-primary-500 text-primary-600 font-semibold rounded-lg hover:bg-primary-50"><Play className="w-6 h-6 mr-2" /> Devam Et</button>
                        )}

                        {status !== 'break' ? (
                            <button onClick={() => {setStatus('break'); timerService.updateStatus(task.id, 'break');}} className="flex items-center justify-center w-32 h-16 bg-white border-2 border-amber-400 text-amber-700 font-semibold rounded-lg hover:bg-amber-50"><Coffee className="w-6 h-6 mr-2" /> Mola Ver</button>
                        ) : (
                            <button onClick={() => {setStatus('running'); timerService.updateStatus(task.id, 'running');}} className="flex items-center justify-center w-32 h-16 bg-amber-500 text-white font-semibold rounded-lg hover:bg-amber-600"><Coffee className="w-6 h-6 mr-2" /> Molayı Bitir</button>
                        )}
...
ActiveTaskTimer.displayName = 'ActiveTaskTimer';
