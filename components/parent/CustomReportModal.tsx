...
const CustomReportModal: React.FC<Props> = ({ tasks, courses, performanceData, summary }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 p-4">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-3xl p-4 sm:p-6 relative overflow-y-auto max-h-[90vh]">
        <h2 className="text-xl sm:text-2xl font-bold mb-2">Durum Raporu</h2>
        <p className="mb-4 text-sm sm:text-base text-gray-700 whitespace-pre-line">{summary}</p>
        <div className="mb-4">
          <span className="text-sm sm:text-base font-semibold">Rapor Grafikleri:</span>
          <div className="flex flex-wrap gap-2 sm:gap-3 mt-2">
            {chartOptions.map(opt => (
              <label key={opt.key} className="flex items-center gap-1 cursor-pointer text-sm sm:text-base">
                <input
                  type="checkbox"
          ...
          {selectedCharts.includes('trend') && (
            <CoursePerformanceTrendChart
              data={performanceData.map(p => ({ 
                period: 'Haftalık', 
                successScore: p.correct || 0, 
                focusScore: p.incorrect || 0, 
        ...
        <button
          className="absolute top-2 sm:top-3 right-2 sm:right-3 text-gray-500 hover:text-red-600 text-xl sm:text-2xl w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 transition-colors"
